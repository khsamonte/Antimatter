...
class HowToScene extends Phaser.Scene {
  constructor() {
    ...
    this.gameInfoTxt = "The objective of the game is to destroy the Mothership and (in the near future) beat all 10 levels.\n\nControls:";

    this.controlInfoTxt = "Movement:\nShoot Lasers:\nUse Turbo:\nWear Shield:\nDrop Bomb:\nHeatseeker:\nAntimatter:";

    this.controlKeysTxt = "Arrow Keys\nC\nZ\nS\t\t(unavailable yet)\nX\t\t(unavailable yet)\nV\t\t(unavailable yet)\nA\t\t(unavailable yet)"

    this.spaceObjectsInfoTxt = "Stars:\nBatteries:\nShields:\n\nAsteroids:\nMeteors:\nBlack Holes:\nWormholes:";

    this.descriptionsInfoTxt = "Heals 2 HP\n+100 mvt for 2 secs\nInvulnerability for 10 secs\n\nDeals 1 dmg (all ships)\nDeals 1 dmg (your ship)\n-50% mvt (and eats all objects)\nTeleports ship to a random location";
  ...
  create() {
    ...
    // Ga,e omfp Info Summary
    this.gameInfo = this.add.text(
      0, 0, this.gameInfoTxt, this.textConfig
    );
    ...
    this.controlText = this.add.text(
      0,
      0,
      this.controlInfoTxt, this.controlTextConfig
    );
    ...
    this.keysText = this.add.text(
      0,
      0,
      this.controlKeysTxt, this.controlTextConfig
    );
    this.keysText.setOrigin(0.5, 0);
    this.alignGrid.placeAtIndex(28, this.keysText);

    // Mechanics
    this.mechanicsText = this.add.text(0, 0, "Objects / Space Objects:", this.textConfig);
